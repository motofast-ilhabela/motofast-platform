...
export default function AppMotoboy() {
  ...
  // Busca por motoboy_id (mais robusto) e filtra pela corrida específica no JS
  async function recarregarCorrida(corridaId) {
    ...
    if (error) { console.error("Erro ao recarregar corrida:", error); return; }
    if (!data) return;

    // Filtra só os pedidos dessa corrida específica
    const pedidosDaCorrida = data.filter(p => p.corrida_id === corridaId);
    if (pedidosDaCorrida.length === 0) return;

    setCorridaAtiva(prev => prev ? {
      ...prev,
      pedidos: pedidosDaCorrida.map(p=>({
        id: p.id,
        empresaNome: p.empresarios?.nome || "Estabelecimento",
  ...
  useEffect(()=>{
    if (!corridaAtiva?.id || !motoboyId) return;

    recarregarCorrida(corridaAtiva.id);

    const canal = supabase
      .channel("corrida-motoboy-"+motoboyId)
      .on("postgres_changes", { event: "*", schema: "public", table: "pedidos" }, () => {
        recarregarCorrida(corridaAtiva.id);
      })
      .subscribe();

    // Rede de segurança — verifica a cada 4s
    const intervalo = setInterval(()=>recarregarCorrida(corridaAtiva.id), 4000);

    return () => { supabase.removeChannel(canal); clearInterval(intervalo); };
  },[corridaAtiva?.id, motoboyId]);
