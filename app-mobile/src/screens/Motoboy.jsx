...
function ModalPedidoDisponivel({ pedido, tipoSom, onAceitar, onRecusar }) {
  ...
  // Adicionado em 07/09/2026 no site: conta de monitoramento vendo um
  // pedido que ainda é prioridade de outro motoboy — toca e mostra igual
  // normal, só que sem poder aceitar até a janela de prioridade passar.
  // `tick` (acima) já força rerender a cada segundo, então esse cálculo
  // atualiza sozinho.
...
export default function Motoboy() {
  ...
  // ele ainda consegue abrir a aba "Cancelados" depois e ver o que
  // aconteceu.
  ...
  useEffect(()=>{
    ...
    async function buscarPedidoReal() {
      if (pedidoRef.current) {
        ...
        if (!verificacao || verificacao.status !== "aguardando") {
          ...
          // Adicionado em 07/09/2026 no site a pedido do Alessandro:
          // motoboys estavam reclamando que o pedido só "sumia" da tela sem
          // explicação quando outro motoboy aceitava primeiro — pareciam
      ...
      // lógica de ofertas_pedido específica de cada motoboy.
      {
        ...
        const candidatosValidos = data.filter(p=>{
          ...
          if (p.prioridade_ate && new Date(p.prioridade_ate).getTime() > agora) {
            // Prioridade por horário específico (adicionada em 06/09/2026 no
            // site) manda mais que o Turno Fixo — se o pedido tem um
            // motoboy_id específico de prioridade, só ELE vê nessa janela,
            // mesmo que eu esteja registrado num Turno Fixo qualquer.
            //
            // Contas de monitoramento (gestão do Alessandro) PASSAM a ver o
            // pedido mesmo sem ser a prioridade da vez — toca e aparece na
            // tela, só que sem poder aceitar até a janela de prioridade
            // passar (ver "_bloqueadoAtePrioridade" logo abaixo). É só pra
            // acompanhamento, nunca fura a fila.
        ...
        // enquanto a prioridade ainda é de outro motoboy.
    ...
    const canal = supabase
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "pedidos" }, (payload) => {
        ...
        // Adicionado em 20/09/2026 no site, a pedido do Alessandro: pedido
        // meu que foi CANCELADO (pelo estabelecimento, na maioria das
        // vezes) enquanto eu já tinha aceitado — antes ficava preso na
        // minha tela até eu atualizar manualmente, e podia acontecer de eu
        // fazer a entrega de algo que o estabelecimento já tinha cancelado
        // e chamado outro motoboy, gerando duplicidade e confusão. Agora
        // tira da tela e avisa na hora, com som (sistema separado do
        // RideAlertService — ver tocarAlarmeCancelamento).
        if (atualizado.status === "cancelado") {
          ...
          // Dispara sempre que um pedido MEU for cancelado, independente
          // de estar ou não achando ele no estado local nesse milissegundo
          // exato — com proteção pra nunca avisar duas vezes do mesmo
          // pedido.
  ...
  // Adicionado em 20/09/2026 no site, junto com a aba "❌ Cancelados": busca
  // o mês atual inteiro (não só hoje) — pra alimentar essa aba com histórico
  // organizado por dia.
  ...
  // motoboy pode parar antes clicando em "Já vi, parar o som", mas se
  // aparecer OUTRO cancelamento depois, o alarme volta a tocar do zero,
  // novo alerta. Reaproveita o mesmo tocarSomEscolhido/tipoSom do site —
  // sistema TOTALMENTE separado do RideAlertService/alarme nativo de
  // corrida nova, que não é tocado por nada aqui.
