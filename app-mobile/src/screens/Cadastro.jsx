...
// Adaptações feitas aqui: navegação via useNavigate (react-router) em vez de
// window.location.href (a versão web usa BrowserRouter; o app nativo usa
// HashRouter, que não navega corretamente com href puro), e o link de "esqueci
// minha senha" aponta pra tela de redefinição na web (ainda não portada pro
// app nativo).
...
// ─── TERMOS EXPANSÍVEIS ──────────────────────────────────────────────────────
function ClausulaItem({ titulo, itens, cor }) {
  const [aberto, setAberto] = useState(false);
  return (
    <div style={{border:`1px solid ${aberto ? cor : "#1f2937"}`,borderRadius:8,marginBottom:6,overflow:"hidden"}}>
      <div onClick={()=>setAberto(x=>!x)}
        style={{padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",background:aberto?"#0d1f14":"#0f172a"}}>
        <span style={{color:aberto?cor:"#9ca3af",fontWeight:700,fontSize:12}}>{titulo}</span>
        <span style={{color:cor,fontSize:14,fontWeight:900}}>{aberto?"▲":"▼"}</span>
      </div>
      {aberto && (
        <div style={{padding:"10px 14px",background:"#0a0f1a",borderTop:`1px solid ${cor}33`}}>
          {itens.map((item, i) => (
            <p key={i} style={{color:"#d1d5db",fontSize:12,lineHeight:1.7,margin:"4px 0",
              paddingLeft: item.startsWith("•") ? 10 : 0}}>
              {item}
            </p>
          ))}
        </div>
...
function TermosExpandiveis({ tipo }) {
  ...
  const clausulasMotoboy = [
    ...
    { titulo: "CLÁUSULA 5 — ISENÇÃO DA PLATAFORMA", itens: ["A MotoFast NÃO SE RESPONSABILIZA por:", "• Acidentes, danos ou mortes durante entregas;", "• Problemas mecânicos no veículo do Motoboy;", "• Perdas financeiras por cancelamentos ou baixo volume;", "• Ações criminosas durante as entregas.", "A Plataforma não garante volume mínimo de pedidos ou renda mínima."] },
    { titulo: "CLÁUSULA 6 — REMUNERAÇÃO E TAXA DA PLATAFORMA", itens: ["A MotoFast aplica uma taxa de intermediação sobre o valor de cada entrega, que pode variar de aproximadamente 3% até o teto máximo de 20%.", "Esse percentual não é fixo em 15% ou 20% para todas as entregas — pode ser, por exemplo, 4%, 5%, 8%, 10% ou 12%, dependendo do valor da taxa de entrega praticada pelo estabelecimento em cada bairro.", "O valor exibido no pedido já é o valor líquido que o Motoboy irá receber, com a taxa da plataforma já descontada.", "Pagamento toda TERÇA-FEIRA via PIX para a chave cadastrada.", "Não há adiantamentos ou antecipações de pagamento."] },
    ...
    { titulo: "CLÁUSULA 8 — DESCREDENCIAMENTO", itens: ["A Plataforma poderá descredenciar o Motoboy sem aviso em caso de:", "• Informações falsas no cadastro;", "• Comportamento inadequado;", "• Atos ilícitos durante as entregas;", "• Descumprimento destes termos."] },
    { titulo: "CLÁUSULA 9 — PRIVACIDADE", itens: ["Dados pessoais usados exclusivamente para operação da plataforma e pagamentos.", "O Motoboy autoriza coleta de geolocalização durante as entregas."] },
    { titulo: "CLÁUSULA 10 — VIGÊNCIA", itens: ["Contrato por prazo indeterminado.", "A Plataforma pode alterar os termos com 7 dias de aviso prévio."] },
  ...
  const clausulasEmpresario = [
    ...
    { titulo: "CLÁUSULA 3 — USO GRATUITO E TAXAS DA PLATAFORMA", itens: ["O uso da Plataforma MotoFast é GRATUITO para o Estabelecimento — não há mensalidade, taxa de adesão ou qualquer cobrança fixa.", "O Estabelecimento pode solicitar quantas entregas quiser, sem limite de quantidade.", "O valor da taxa de entrega é pago pelo cliente final no momento do pedido, e o Estabelecimento repassa esse valor à MotoFast.", "A MotoFast aplica uma margem sobre o valor dessa taxa de entrega, podendo variar de aproximadamente 3% até o teto máximo de 20%, dependendo do valor praticado em cada bairro/rota.", "Esse percentual não é fixo em 15% ou 20% para todas as taxas — pode ser, por exemplo, 4%, 5%, 8%, 10% ou 12%, entre outros valores.", "O Estabelecimento repassa esses valores à MotoFast de uma das duas formas:", "• Pagamento diário: as entregas realizadas em um dia são repassadas até as 22h do dia seguinte;", "• Pagamento semanal: as entregas realizadas de terça-feira a domingo se acumulam e são repassadas até as 22h da segunda-feira seguinte."] },
    { titulo: "CLÁUSULA 4 — BLOQUEIO POR INADIMPLÊNCIA", itens: ["No horário de vencimento de cada modalidade (diariamente para quem está no plano de repasse diário, ou toda segunda-feira para quem está no plano semanal), a Plataforma verifica, por volta das 22h, se o repasse das taxas de entrega foi identificado.", "Caso o pagamento não seja identificado até esse horário, a conta do Estabelecimento será BLOQUEADA automaticamente por inadimplência.", "Durante o bloqueio, novos pedidos não poderão ser solicitados.", "O desbloqueio ocorre somente após a regularização integral dos valores pendentes."] },
    ...
    { titulo: "CLÁUSULA 7 — OBRIGAÇÕES", itens: ["• Efetuar os repasses das taxas de entrega nos prazos estabelecidos;", "• Tratar motoboys com respeito;", "• Manter dados de contato atualizados;", "• Não usar a plataforma para fins ilegais."] },
    { titulo: "CLÁUSULA 8 — RESCISÃO", itens: ["O Estabelecimento pode cancelar a qualquer momento, quitando valores pendentes.", "A Plataforma pode rescindir sem aviso em caso de uso ilegal ou inadimplência."] },
    { titulo: "CLÁUSULA 9 — PRIVACIDADE", itens: ["Dados usados exclusivamente para operação e cobranças.", "Não serão comercializados a terceiros."] },
    { titulo: "CLÁUSULA 10 — FORO", itens: ["Foro da Comarca de Ilhabela/SP para quaisquer controvérsias."] },
  ];
  ...
  return (
    <div style={{marginBottom:12}}>
      <div style={{color:cor,fontSize:12,fontWeight:700,marginBottom:8}}>📋 Termos de Uso — Clique em cada cláusula para ler</div>
      {clausulas.map((c, i) => (
        <ClausulaItem key={i} titulo={c.titulo} itens={c.itens} cor={cor}/>
      ))}
    </div>
  );
}
          ...
          <div style={{background:"#0f172a",border:"1px solid #374151",borderRadius:10,padding:"14px 16px",marginBottom:14}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <label htmlFor="termos-emp" style={{color:"#d1d5db",fontSize:13,cursor:"pointer"}}>
                Li e aceito todos os Termos de Uso do MotoFast
              </label>
            </div>
  ...
  async function entrar() {
    ...
    if (!perfil) {
      ...
      if (perfilOutraTabela) {
        ...
        setErro(`Essa conta é de ${nomeOutroTipo}, não de ${c.label.toLowerCase()}. Volta pra tela inicial e entra pelo login de ${nomeOutroTipo}.`);
      } else {
        ...
        <div style={{width:"100%",maxWidth:460}}>
          ...
          {tela==="inicio" && (
            <div style={{textAlign:"center"}}>
              ...
              <div style={{marginTop:16}}>
                <a href={`https://wa.me/${SUPORTE_TEL}?text=Ol%C3%A1!%20Estou%20no%20app%20do%20MotoFast%20e%20tenho%20uma%20d%C3%BAvida%20antes%20de%20me%20cadastrar%20%E2%9A%A1`}
          ...
          {tela==="login-emp" && (
            <div style={{background:"#111827",border:"1px solid #1f2937",borderRadius:16,padding:28}}>
              <TelaLogin tipo="empresario" onCadastrar={()=>setTela("cad-emp")}/>
            </div>
          )}
          ...
          {tela==="login-mb" && (
            <div style={{background:"#111827",border:"1px solid #1f2937",borderRadius:16,padding:28}}>
              <TelaLogin tipo="motoboy" onCadastrar={()=>setTela("cad-mb")}/>
            </div>
          )}
