...
// Cópia adaptada de Admin.jsx da plataforma web (ver CLAUDE.md — mudanças de
// regra de negócio precisam ser replicadas manualmente entre as duas
          ...
          <Card style={{marginBottom:14,padding:"14px 16px",background:"#0f172a"}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
              ...
              <button onClick={async()=>{
                ...
                const resp = await fetch(`${WEB_APP_URL}/api/redefinir-senha`, {
                  method: "POST",
                  headers: {"Content-Type":"application/json"},
              ...
              <div>
                <Card style={{marginBottom:14,background:"#0d3d2e",border:"1px solid #34d399"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8,marginTop:2,flexWrap:"wrap"}}>
                    ...
                    {entregasDia.length>0 && statusDataSelecionadaEmp==="pago" && <Tag label="✅ Pago" cor="#34d399"/>}
                    {entregasDia.length>0 && statusDataSelecionadaEmp==="pendente" && <Tag label="⚠️ Pendente" cor="#fbbf24"/>}
                  </div>
                </Card>
            ...
            <div>
              ...
              {empSel.planoPagamentoMotoboy==="semanal" && (()=>{
                ...
                historico.filter(e=>e.empresarioId===empSel.id&&e.status==="Entregue").forEach(e=>{
                  ...
                  porSemanaMapPag[e.semana] += e.taxaEmpresario;
                });
                const totalPendenteGeral = Object.entries(porSemanaMapPag).reduce((s,[sem,tot])=>{
                  const pg = empSel.pagamentosSemanais?.[sem] || 0;
                  return s + Math.max(0, tot-pg);
                }, 0);
                const temPendenciaAntiga = totalPendenteGeral > pendenteSemanaAtual + 0.01;
                return (
                  <Card style={{padding:"14px 16px",background:"#0f172a"}}>
                    ...
                    <div style={{background:temPendenciaAntiga?"#3d2a00":"#0d3d2e",border:`1px solid ${temPendenciaAntiga?"#f59e0b":"#34d399"}`,borderRadius:8,padding:"10px 14px",marginBottom:12}}>
                      ...
                      {temPendenciaAntiga && (
                        <div style={{color:"#fbbf24",fontSize:11,marginTop:4}}>⚠️ Inclui semana(s) anterior(es) em aberto — desce até "📅 Semanas anteriores" na aba "Por Dia" pra ver e marcar cada uma.</div>
                      )}
                    </div>
...
function CorridasAtivas({ corridasAtivas, onRecarregar, motoboys }) {
  ...
  // Ajustado em 07/09/2026 no site: inclui também os já "entregue" que
  // ainda pertencem a uma corrida com pelo menos um pedido não finalizado,
  // pra dar pra ver "✅ Finalizado" item por item em vez do card sumir só
...
export default function Admin() {
  ...
  async function carregarTudo(silencioso=false) {
    ...
    try {
      ...
      // Adicionado em 07/09/2026 no site: dentro de uma corrida com vários
      // pedidos, o motoboy agora grava cada entrega individualmente assim
      // que confirma (não espera terminar todas — ver
      // entregarItemIndividual no Motoboy.jsx). Pra continuar mostrando a
      // corrida inteira até o motoboy finalizar todas — só que já marcando
      // quais já foram entregues — busca também os pedidos "entregue" que
      // pertencem a uma corrida que ainda tem pelo menos um pedido ativo.
  ...
  async function aprovar(tipo, id, userId) {
    ...
    if (userId) {
      try {
        await fetch(`${WEB_APP_URL}/api/confirmar-email`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
