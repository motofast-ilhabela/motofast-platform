...
export default function AppMotoboy() {
  ...
  // Mantém a corrida sempre sincronizada com o banco — detecta pedidos extras
  // adicionados pelo empresário na mesma corrida, e atualiza status (entregue/cancelado)
  async function recarregarCorrida(corridaId) {
    if (!corridaId) return;
    console.log("🔍 Recarregando corrida:", corridaId);
    ...
    if (error) console.error("❌ Erro ao recarregar corrida:", error);
    console.log("📦 Pedidos encontrados:", data?.length, data);
  ...
  useEffect(()=>{
    if (!corridaAtiva?.id) return;

    // Atualiza imediatamente ao entrar na corrida
    recarregarCorrida(corridaAtiva.id);

    const canal = supabase
      .channel("corrida-motoboy-"+corridaAtiva.id)
      .on("postgres_changes", { event: "*", schema: "public", table: "pedidos" }, (payload) => {
        // Filtra no JS em vez de depender do filtro do Supabase (mais confiável)
        if (payload.new?.corrida_id === corridaAtiva.id || payload.old?.corrida_id === corridaAtiva.id) {
          recarregarCorrida(corridaAtiva.id);
        }
      })
      .subscribe();

    // Rede de segurança — verifica a cada 5s
    const intervalo = setInterval(()=>recarregarCorrida(corridaAtiva.id), 5000);
  ...
  async function aceitar() {
    ...
    const { data: corridaDB, error: corridaError } = await supabase
      .from("corridas")
      .insert({ motoboy_id: motoboyId, status: "ativa" })
      .select()
      .single();

    console.log("🏍️ Corrida criada:", corridaDB, "erro:", corridaError);
    ...
    console.log("✅ Corrida ativa definida com id:", corridaDB?.id);
    setCorridaAtiva({
