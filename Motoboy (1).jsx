...
export default function AppMotoboy() {
  ...
  useEffect(()=>{
    ...
    const canal = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "pedidos", filter: `corrida_id=eq.${corridaAtiva.id}` }, () => {
        recarregarCorrida(corridaAtiva.id);
      })
      .subscribe();

    // Rede de segurança — verifica a cada 6s
    const intervalo = setInterval(()=>recarregarCorrida(corridaAtiva.id), 6000);
